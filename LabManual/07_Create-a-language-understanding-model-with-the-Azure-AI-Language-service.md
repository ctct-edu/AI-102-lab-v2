...
1. 上部の検索フィールドで **Language** を検索します。結果で **Language** の下の **作成** を選択します。
1. **リソースの作成を続行** を選択します。
1. 次の設定を使用してリソースをプロビジョニングします：
    - **サブスクリプション**: *Azure サブスクリプション*。
    - **リソースグループ**: *リソースグループを選択または作成*。
    - **リージョン**: *米国西部 2*
    - **名前**: Lab07-yyyymmdd-任意の文字列（お名前等）※例:Lab07-20250801-doi
...
### 事前構築済み（Prebuilt）エンティティの追加

Azure AI Language サービスは、会話型アプリケーションで一般的に使用される一連の *事前構築済み（Prebuilt）* エンティティを提供します。

1. Language Studio で **スキーマ定義** ページに戻り、**エンティティ** タブで **＋ 追加** を選択して新しいエンティティを追加します。

1. **エンティティの追加** ダイアログボックスで、エンティティ名を `Date` と入力し、**事前構築済み（Prebuilt）** エンティティタブを選択します。次に **エンティティの追加** を選択します。

1. **Date** エンティティのページで、**学習済み** セクションで **必須ではない** が選択されていることを確認します。次に、***事前構築済み（Prebuilt）** セクションで **＋ 新しい*事前構築済みの追加** を選択します。

1. **事前構築の選択** リストで **DateTime** を選択し、**保存** を選択します。

1. 事前構築済みエンティティを追加した後、**データラベリング** ページに戻ります。
...
3. この演習では、**07-language** フォルダを使用します。
...
1. Visual Studio Code の **Explorer** ペインで **07-language** フォルダーに移動し、言語の好みに応じて **CSharp** または **Python** フォルダーを展開し、その中にある **clock-client** フォルダーを展開します。各フォルダーには、Azure AI Language 質問応答機能を統合するアプリの言語固有のファイルが含まれています。
2. コードファイルを含む **clock-client** フォルダーを右クリックし、統合ターミナルを開きます。次に、言語の好みに応じて適切なコマンドを実行して Azure AI Language 会話型言語理解 SDK パッケージをインストールします：
...
3. **Explorer** ペインで **clock-client** フォルダー内の構成ファイルを開きます。
    - **C#**: appsettings.json
    - **Python**: .env
...
1. **clock-client** フォルダーにはクライアントアプリケーションのコードファイルが含まれています：
    - **C#**: Program.cs
