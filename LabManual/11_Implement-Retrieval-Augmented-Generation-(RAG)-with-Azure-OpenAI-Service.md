...
2. 次の設定で **Azure OpenAI** リソースを作成します：
    ...
    - **リージョン**: *次のリージョンからランダムに選択*\*
        ...
        - スイス北部
        - 英国南部
    - **名前**: Lab11-yyyymmdd-任意の文字列（お名前等）※例:Lab11-20250801-doi
    - **価格レベル**: Standard S0
...
3. Azure OpenAI リソースがプロビジョニングされている間に、次の設定で **Azure AI Search** リソースを作成します：
    ...
    - **サービス名**: lab11-search-yyyymmdd-任意の文字列（お名前等）※例:lab11-search-20250801-doi
...
4. Azure AI Search リソースがプロビジョニングされている間に、次の設定で **ストレージアカウント** リソースを作成します：
    - **サブスクリプション**: *Azure OpenAI リソースをプロビジョニングしたサブスクリプション*
    - **リソースグループ**: *Azure OpenAI リソースをプロビジョニングしたリソースグループ*
    - **ストレージアカウント名**: lab11strageyyyymmdd任意の文字列（お名前等）※例:lab11strage20250801doi
...
1. Visual Studio Code で、**Labfiles/11-use-own-data/data** フォルダを開き、このフォルダに含まれている .pdf パンフレットファイルを確認します。これらのファイルを後の手順でBlobコンテナーにアップロードします。
1. Azure ポータルで、ストレージアカウントに移動し、**ストレージブラウザ** ページを表示します。
1. **Blob コンテナー** を選択し、`margies-travel` という名前の新しいコンテナーを追加します。
1. **margies-travel** コンテナーを選択し、**Labfiles/11-use-own-data/data** フォルダにある .pdf パンフレットファイルを Blob コンテナーのルートフォルダにアップロードします。
...
1. Azure AI Foundry ポータルの **デプロイ** ページで、既存のモデルデプロイメントを表示します。次に、**＋ モデルのデプロイ** から **基本モデルのデプロイ** を選択し、モデル選択画面から **text-embedding-ada-002** モデルの新しいベースモデルデプロイメントを作成します：
    - **デプロイメント名**: *一意の名前を入力* ※デフォルトの名前で構いません。
    - **デプロイメントタイプ**: Standard
...
3. この演習では、**11-use-own-data** フォルダを使用します。

## アプリケーションの構成

C# と Python の両方のアプリケーションが提供されており、同じ機能を備えています。まず、Azure OpenAI リソースを使用できるようにアプリケーションのいくつかの重要な部分を完了します。

1. Visual Studio Code の **Explorer** ペインで **11-use-own-data** フォルダーを展開し、希望する言語に応じて **CSharp** または **Python** フォルダーを展開します。各フォルダーには、Azure OpenAI 機能を統合するアプリの言語固有のファイルが含まれています。
...
1. **Explorer** ペインで **CSharp** または **Python** フォルダー内のコードファイルを開き、コメント ***Configure your data source*** を Azure OpenAI SDK ライブラリを追加するコードに置き換えます：
