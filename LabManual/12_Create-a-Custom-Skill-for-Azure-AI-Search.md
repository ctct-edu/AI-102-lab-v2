...
1. Azure AI Search リソースのページに移動し、左メニューの検索管理を展開し、**インデックス** を選択して 作成されたインデクスを選択します。
...
3. **概要** ページで、ページの下部にある **Create in Azure Portal** を選択し、次の設定で新しい関数を作成します：
    - **テンプレートを選択**
        - **テンプレート**: HTTP Trigger    
    - **テンプレートの詳細**：
        - **関数名**: wordcount
