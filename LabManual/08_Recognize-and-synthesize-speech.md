...
1. 上部の検索フィールドで **音声** を検索し、**Enter** キーを押して、結果で **音声** の下の **作成** を選択します。
1. 次の設定を使用してリソースを作成します：

    - **サブスクリプション**: *Azure サブスクリプション*
    - **リソースグループ**: *リソースグループを選択または作成*
    - **リージョン**: *利用可能なリージョンを選択*
    - **名前**: Lab08-yyyymmdd-任意の文字列（お名前等）※例:Lab08-20250801-doi
...
3. この演習では、**08-speech** フォルダを使用します。

## アプリケーションの構成

C# と Python の両方のアプリケーションが提供されており、同じ機能を備えています。まず、Azure AI Speech リソースを使用できるようにアプリケーションのいくつかの重要な部分を完了します。

1. Visual Studio Code の **Explorer** ペインで **08-speech** フォルダーに移動し、言語の好みに応じて **CSharp** または **Python** フォルダーを展開し、その中にある **speaking-clock** フォルダーを展開します。各フォルダーには、Azure AI Speech 機能を統合するアプリの言語固有のファイルが含まれています。
1. コードファイルを含む **speaking-clock** フォルダーを右クリックし、統合ターミナルを開きます。次に、言語の好みに応じて適切なコマンドを実行して Azure AI Speech SDK パッケージをインストールします：
...
1. **Explorer** ペインで **speaking-clock** フォルダー内の構成ファイルを開きます：
...
1. **speaking-clock** フォルダーにはクライアントアプリケーションのコードファイルが含まれています：
...
1. 変更を保存し、**speaking-clock** フォルダーの統合ターミナルに戻り、次のコマンドを入力してプログラムを実行します：
