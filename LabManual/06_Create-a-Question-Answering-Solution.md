...
ナレッジベースをゼロから作成することもできますが、既存の FAQ ページやドキュメントから質問と回答をインポートすることが一般的です。今回、デスクトップに配置されているPDFファイルからデータをインポートし、一般的な会話のやり取りをサポートするために事前定義された「雑談」質問と回答もインポートします。

1. 質問応答プロジェクトの **ソースの管理** ページで、**╋ ソースを追加** リストから **ファイル** を選択します。次に **ファイルの追加** ダイアログボックスで **╋ ファイルを追加** を選択し、デスクトップに配置されている **LearnFAQ.pdf** ファイルを選択してから **すべて追加** を選択してナレッジベースに追加します。
    - **名前**: `Learn FAQ Page`
...
1. メッセージ `What are Microsoft credentials?` を入力します。作成した回答が返され、フォローアップ プロンプトのリンクが表示されるはずです。
...
1. Language Studio の **LearnFAQ** プロジェクトで、左側のナビゲーション メニューから **デプロイ ナレッジベース（サポート技術情報の展開）** ページを選択します。
...
1. Visual Studio Code の **Explorer** ペインで、**06-qna** フォルダーに移動し、希望する言語に応じて **CSharp** または **Python** フォルダーとその中の **qna-app** フォルダーを展開します。各フォルダーには、Azure AI Language 質問応答機能を統合するための言語固有のファイルが含まれています。
